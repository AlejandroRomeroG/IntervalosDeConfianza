...
ConfianzaMediasUnaColaDerecha <- function(datos,certeza){
  n <- length(datos)
  media <- mean(datos)
  desvest <- sd(datos)
  alfa <- 1-certeza
  ...
  vecindadderecha <- media-t*(desvest/sqrt(n))
  
  paste("P( ", greeks("mu") ," < ",vecindadderecha, " ) = ", certeza)
}
...
ConfianzaProporcionesUnaColaDerecha <- function(k,n,certeza){
  alfa <- 1-certeza
  z <- qnorm(alfa)
  estimador <- k/n
  ...
  paste("P( ", "θ" ," < ",vecindadderecha, " ) = ", certeza)
}
